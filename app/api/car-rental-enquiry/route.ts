...
export async function POST(req: Request) {
  try {
    ...
    if (!fullName || !mobileNumber || !pickupLocation || !pickupDate || !rentalType) {
    ...
    const mailOptions = {
      from: `"Ambica Travels Car Rental" <${process.env.SMTP_USER}>`,
      to: process.env.CONTACT_EMAIL_TO || process.env.SMTP_USER,
      replyTo: email || undefined,
      subject: `New car rental enquiry: ${fullName}`,
      text: `
Name: ${fullName}
Mobile: ${mobileNumber}
Email: ${email || "—"}
...
${additionalMessage || "—"}
    ...
    console.error("Car rental enquiry error:", error);
    return NextResponse.json(
