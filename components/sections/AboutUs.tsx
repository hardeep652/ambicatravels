    ...
    <section id="about" className="section-py bg-white">
      <div className="container-px mx-auto max-w-7xl">
        ...
        <div className="mt-16 grid gap-6 md:grid-cols-3">
          {[
            { icon: "shield", title: "Trusted Since 2009", desc: "15+ years of referrals & repeat travelers." },
            { icon: "headset", title: "24×7 Trip Support", desc: "A real person answers — before, during, after." },
            { icon: "wallet", title: "Transparent Pricing", desc: "No hidden fees; everything in writing." },
            { icon: "map", title: "On‑Ground Network", desc: "Verified partners in 120+ destinations." },
            { icon: "card", title: "Secure Payments", desc: "Razorpay‑protected checkout." },
            { icon: "users", title: "Family‑First Approach", desc: "Itineraries designed for all ages." },
          ].map((item, i) => (
            <div
              key={i}
            ...
            >
              <div className="mb-3 text-emerald-500">
                ...
                <span className="text-3xl">{item.icon === "shield" && "🛡️"}</span>
                <span className="text-3xl">{item.icon === "headset" && "🎧"}</span>
                <span className="text-3xl">{item.icon === "wallet" && "💳"}</span>
                <span className="text-3xl">{item.icon === "map" && "🗺️"}</span>
                <span className="text-3xl">{item.icon === "card" && "🔐"}</span>
                <span className="text-3xl">{item.icon === "users" && "👨‍👩‍👧‍👦"}</span>
              </div>
              <h3 className="font-heading text-lg font-semibold text-navy-900">{item.title}</h3>
